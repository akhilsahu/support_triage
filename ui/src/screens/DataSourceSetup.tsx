...
import { Loader2, Plus, Plug, Server, Trash2 } from 'lucide-react'
...
interface DataSourceCard { id: string; name: string; apiUrl: string; authType: string; method: string; active: boolean }
...
export function DataSourceSetup() {
  ...
  const load = async () => {
    ...
    try {
      ...
      setSources(tools.map((tool: any) => {
        ...
        return { id: tool.id, name: tool.display_name || tool.name, apiUrl: `${connection.base_url || ''}${tool.path || ''}`, authType: connection.auth_type || 'none', method: tool.method, active: tool.status === 'active' }
      }))
  ...
  if (adding) return <DataSourceWizard chatbotId={chatbotId} agents={agents} onCancel={closeWizard} onComplete={async () => { await load(); closeWizard() }} />
  ...
  return <div className="max-w-4xl mx-auto p-6 space-y-6">
    ...
    {!sources.length ? <div className="rounded-2xl border border-dashed border-gray-300 dark:border-gray-800 p-12 text-center"><Plug className="mx-auto mb-3 h-8 w-8 text-indigo-500" /><h3 className="text-sm font-semibold">No data sources connected</h3><p className="mx-auto mt-1 mb-5 max-w-sm text-xs text-gray-500">Connect a REST API so assigned agents can retrieve live information.</p><Button onClick={() => setAdding(true)}><Plus className="h-4 w-4" /> Add data source</Button></div>
      : <div className="grid gap-4 md:grid-cols-2">{sources.map(source => <article key={source.id} className="rounded-xl border border-gray-200 dark:border-gray-800 bg-white dark:bg-gray-900 p-4 shadow-sm"><div className="flex items-start justify-between gap-3"><div className="flex min-w-0 items-center gap-2"><span className="rounded-lg bg-indigo-50 dark:bg-indigo-950/40 p-2 text-indigo-600"><Server className="h-4 w-4" /></span><h3 className="truncate text-xs font-semibold">{source.name}</h3></div><span className={`rounded px-2 py-1 text-[10px] font-bold ${source.active ? 'bg-emerald-50 text-emerald-700' : 'bg-gray-100 text-gray-500'}`}>{source.active ? 'ACTIVE' : 'INACTIVE'}</span></div><div className="my-3 rounded-lg bg-gray-50 dark:bg-gray-950 p-3 font-mono text-[11px]"><strong className="text-indigo-600">{source.method}</strong><p className="mt-1 break-all text-gray-500">{source.apiUrl}</p></div><div className="flex items-center justify-between border-t border-gray-100 dark:border-gray-800 pt-3"><span className="text-[10px] text-gray-500">Auth: {source.authType.toUpperCase()}</span><button onClick={async () => { if (!confirm('Disable this data source tool?')) return; await apiClient.updateDataSourceTool(source.id, { status: 'disabled' }); await load() }} className="inline-flex items-center gap-1 text-xs font-medium text-red-500"><Trash2 className="h-3.5 w-3.5" /> Disable</button></div></article>)}</div>}
  </div>
}
