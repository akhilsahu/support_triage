    ...
    <div className="group space-y-1.5">
      <div className="flex justify-between items-baseline px-4">
        <div className="flex items-center gap-1.5">
          <label className="block text-xs font-bold tracking-wider text-slate-350">
            {label}
          </label>
          {tooltip && (
            <div className="relative flex items-center group/tooltip">
              <span className="cursor-help p-0.5 text-slate-500 hover:text-indigo-400 transition-colors">
                <svg className="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2.5}>
                  <path strokeLinecap="round" strokeLinejoin="round" d="M9.879 7.519c1.171-1.025 3.071-1.025 4.242 0 1.172 1.025 1.172 2.687 0 3.712-.203.179-.43.326-.67.442-.745.361-1.45.999-1.45 1.827v.75M21 12a9 9 0 11-18 0 9 9 0 0118 0zm-9 5.25h.008v.008H12v-.008z" />
                </svg>
              </span>
              <div className="absolute bottom-full left-1/2 -translate-x-1/2 mb-2 w-52 p-3 bg-slate-900 border border-white/10 text-xs text-slate-200 rounded-2xl shadow-2xl opacity-0 scale-90 pointer-events-none group-hover/tooltip:opacity-100 group-hover/tooltip:scale-100 transition-all duration-200 z-50 text-center font-medium normal-case leading-normal tracking-normal backdrop-blur-md">
      ...
      <div className="relative flex items-center">
        <div className="absolute left-4 flex items-center justify-center pointer-events-none">
          <Icon className="w-4 h-4 text-slate-500 group-focus-within:text-indigo-400 transition-colors duration-300" />
        </div>
        <input
          type={inputType}
          value={value}
          ...
          required={required}
          className="w-full pl-11 pr-11 py-2.5 text-sm bg-[#181b28] border border-white/10 rounded-full text-white placeholder-slate-500 focus:outline-none focus:border-indigo-500/50 focus:ring-4 focus:ring-indigo-500/10 shadow-[0_4px_15px_rgba(0,0,0,0.2)] transition-all duration-300 ease-out font-medium"
        />
        {isPassword && (
          <button
            type="button"
          ...
          {(['login', 'register'] as Tab[]).map((tabKey) => (
            <button
              ...
              className={`flex-1 py-1.5 px-3 rounded-full text-xs font-bold tracking-wide transition-all duration-300 ${
                tab === tabKey
                  ? 'bg-[#181b28] text-white shadow-sm'
            ...
            {tab === 'login' ? (
              <>
                ...
                <button
                  ...
                  className="text-indigo-400 font-bold hover:underline transition-colors focus:outline-none"
                >
                  Create space
                </button>
              </>
