...
Usage examples:
  ...
  python -m app.utils.ai.document_ingestion

  # Re-ingest a specific document by its Postgres UUID
  python -m app.utils.ai.document_ingestion --doc-id <uuid>

  # Re-ingest all docs for a specific org
  python -m app.utils.ai.document_ingestion --org-id <uuid>

  # Re-ingest raw text directly (no DB lookup)
  python -m app.utils.ai.document_ingestion --raw-text "Some content here" --client-id <space_id> --filename manual.txt

  # Re-ingest from a local file
  python -m app.utils.ai.document_ingestion --file /path/to/doc.pdf --client-id <space_id>

  # Reset a collection before re-ingesting
  python -m app.utils.ai.document_ingestion --reset-collection client_documents

  # Dry run (show what would be ingested, no writes)
  python -m app.utils.ai.document_ingestion --dry-run

  # Filter by doc_type
  python -m app.utils.ai.document_ingestion --doc-type policy

  # Limit batch size
  python -m app.utils.ai.document_ingestion --batch-size 50
"""
...
async def reingest_from_db(
    ...
    if dry_run:
        for d in docs:
            print(f"[DRY RUN] Would re-ingest doc_id={d.id} org={d.space_id} source={d.source} len={len(d.content)}")
        return
    ...
    for i in range(0, len(docs), batch_size):
        ...
        for d in batch:
            ...
            for c in chunks:
                chunk_id = f"{d.space_id}:{d.id}:{c['chunk_index']}"
                ids.append(chunk_id)
                texts.append(c["text"])
                metas.append({
                    "client_id":   str(d.space_id) if d.space_id else "",
                    "doc_id":      str(d.id),
                    "filename":    d.source,
                    ...
                    "section":     c["section"],
                    "strategy":    "reingest",
                    "space_id":      str(d.space_id) if d.space_id else "",
                    "session_id":  "",
