"""
OrchestraSupport Chatbot-Level Orchestration PoC

This script demonstrates the complete dynamic orchestration flow at the Chatbot tier:
1. Resolves active agents (built-in configurations + active custom agents) for a chatbot.
2. Performs intent triage & routing (bypassing triage if only one specialist is active).
3. Assembles sandboxed prompt context (base prompt, system prompt, attached PromptSkills).
4. Resolves RAG and external data scopes matching the tenant organization.
5. Generates the grounded LLM response and logs transactions to PostgreSQL and Redis.

Usage:
  Activate virtual environment and run from the project root:
  python -m app.orchestra.poc
"""
